...
def analyze_route(lat: float, lng: float, duration_mins: float, distance_km: float, route_type: int = 0, start_name: str = "Origin", end_name: str = "Destination"):
    try:
        ...
        # 2. 🚀 AGENTIC AI SPATIAL & TRAFFIC REASONING
        ...
        if client:
            ...
            "transit_suggestion": (string) A 1-2 sentence casual message suggesting eco-friendly public transit (metro, bus, train) between these areas. Start exactly with: "Bro, if you're not in a hurry..."
            ...
            try:
                ...
                print(f"🤖 Route Analysis [{current_time_str}] -> Water: {has_water_body}, Garbage: {has_garbage_dump}, Traffic: {has_heavy_traffic}")
        ...
        if has_garbage_dump:
            ...
            pm25 += 15 
            pm10 += 25
            warning_tags.append("☣️ Toxic Waste Dump Nearby: +10 AQI (AI detected landfill within 5km).")
            env_reason = "🗑️ Warning: Route passes near a known open dumping ground. Harmful methane detected."
        
        if has_water_body:
            aqi = max(10, aqi - 10)
            pm25 = max(5, pm25 - 10) 
            warning_tags.append("🌊 Cooling Water Body Nearby: -10 AQI (Natural air filtration).")
            env_reason = "🌿 Route parallels a significant water body. Natural breeze filters out PM2.5."

        if has_heavy_traffic:
            aqi += 5
            no2 += 20 
            warning_tags.append(f"🚨 High Traffic Detected at {current_time_str.split(' on ')[0]}: +5 AQI (Harmful exhaust density).")
        ...
        if aqi > 80 or pm25 > 25:
            medical_alert = f"⚠️ SEVERE RISK: This route forces {round(pm25, 1)} μg/m³ of micro-particles deep into your lung alveoli. Commuting this way daily increases asthma and COPD risks."
        else:
        ...
        return {
            "status": "success",
            "aqi": round(aqi),
            "health": { "life_lost_mins": life_lost_mins, "saved_life_mins": saved_life_mins, "duration": duration_mins, "distance": distance_km },
            "reason": env_reason,
